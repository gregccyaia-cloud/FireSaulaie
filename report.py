...
def build(c,out):
 ...
 d.add_heading('5. Choix des paramètres et hypothèses',1);d.add_paragraph('La hauteur de foyer constitue un paramètre de sensibilité du modèle géométrique. Les valeurs 10 m, 15 m et 20 m représentent des enveloppes de hauteur de flamme. Elles s’inscrivent dans la logique du guide Cerema, qui distingue le choix du scénario, de la courbe thermique et la caractérisation géométrique du foyer ; ces trois valeurs restent des hypothèses de l’étude et non des valeurs imposées par le guide.');table(d,c['params'])
 ...
 d.add_heading('8. Impact structurel',1);d.add_paragraph(c['pilot']);d.add_paragraph('Les résistances à chaud sont estimées par Ft,Rd,θ = ky,θ × Ft,Rd et les ratios par ηfi = NQP / Ft,Rd,θ.');table(d,c['struct'],6.8);d.add_paragraph(c['maxtext']);fig(d,c['f14'],'Figure 14 - Ratio de vérification de la suspente secondaire.');fig(d,c['f15'],'Figure 15 - Ratio de vérification du câble principal clos.')
 d.add_heading('9. Conclusion et poursuite',1);d.add_paragraph(c['conclusion'])
 d.add_page_break();d.add_heading('Annexe A - Détail du processus de calcul',1);d.add_paragraph('L’annexe illustre le calcul du cas critique à 30 min. La température à chaque pas résulte de la mise à jour de c_a(θa), des flux convectif et radiatif et de l’intégration sur Δt = 5 s.');table(d,c['steps']);fig(d,c['integration'],'Figure A.1 - Intégration temporelle du cas critique.');table(d,c['worst'])
