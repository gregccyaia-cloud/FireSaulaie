...
from config import *
def cp_steel(theta):
    """Chaleur spécifique acier carbone, NF EN 1993-1-2 §3.4.1.2."""
...
def integrate(t,tg,amv):
    """Bilan concentré acier non protégé, NF EN 1993-1-2 §4.2.5.1."""
    n=len(t);ta=np.full(n,AMBIENT_C);qc=np.zeros(n);qr=np.zeros(n);qn=np.zeros(n);cp=np.zeros(n);dta=np.zeros(n);cp[0]=cp_steel(ta[0])
    for i in range(1,n):
